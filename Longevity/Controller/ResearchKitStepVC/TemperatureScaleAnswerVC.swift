...
class TemperatureScaleAnswerVC: ContinuousScaleAnswerVC {
        ...
        AppSyncManager.instance.healthProfile.addAndNotify(observer: self) {
            ...
            if let unit = AppSyncManager.instance.healthProfile.value?.unit {
                ...
                case .metric:
                    ...
                    self?.slider.minimumValueImage = "\(String(format: "%.2f", minCelsius)) \u{00B0} C".toImage()
                    self?.slider.maximumValueImage = "\(String(format: "%.2f", maxCelsius)) \u{00B0} C".toImage()
                    if let localSavedAnswer =
                        ...
                        self?.sliderLabel.text = "\(String(format:"%.2f",localSavedAnswer)) \u{00B0} C"
                        self?.continueButton.isEnabled = true
                    } else {
                        self?.slider.setValue(Float(minCelsius), animated: true)
                        self?.sliderLabel.text = "\(String(format:"%.2f",minCelsius)) \u{00B0} C"
                    }
                case .imperial:
                    ...
                    if let localSavedAnswer =
                        ...
                        self?.sliderLabel.text = "\(String(format:"%.2f",localAnswerFahrenheit)) \u{00B0} F"
                        self?.continueButton.isEnabled = true
                    }else {
                        self?.slider.setValue(Float(minFahrenheit), animated: true)
                        self?.sliderLabel.text = "\(String(format:"%.2f",minFahrenheit)) \u{00B0} F"
                    }
                }
            }
        }
    ...
    override func handleSliderValueChanged(_ sender: UISlider) {
        ...
        if let unit = AppSyncManager.instance.healthProfile.value?.unit {
            switch unit {
            case .metric:
                self.sliderLabel.text = "\(String(format:"%.2f",sender.value)) \u{00B0} C"
            case .imperial:
                self.sliderLabel.text = "\(String(format:"%.2f",sender.value)) \u{00B0} F"
            }
        }
